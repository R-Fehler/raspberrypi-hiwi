...
class Recorder(object):
    ...
    def record_start(self, device):
        ...
        logging.debug("start recording with chunk length of " +
                      str(self.__rectime/60)+" min.")
        # logging.debug("-"*40)
...
#          time.sleep(10.0 / 100.0)
    ...
    def SetPathDetails(self, rectime, directory, prefix='rec', suffix='wav'):
        ...
        self.__rec_frames = int(
            self.rate * self.__rectime / self.frames_per_buffer)
        ...
        if self.__running_index > 0:
            os.system("rm " + os.path.join(self.__pathname,
                                           "*_id_" + self.__prefix))
            os.system("touch " + os.path.join(self.__pathname,
                                              str(self.__running_index) + "_id_" + self.__prefix))
        logging.debug("Index for filenames is now: " +
                      str(self.__running_index))
    ...
    def _get_new_filename(self, broken=False, setNewTime=True, frames=0):
        if setNewTime is True:  # and self.__file_time != 0:
            # relative time from start # time.datetime.now()
            self.__file_time[0] = time.perf_counter()
    ...
    def CleanAfterBrokenRecord(self):
        ...
        if self.frames/self.frames_per_buffer < 5:
            logging.error('There are only %i frames; dropping file.' %
                          self.frames/self.frames_per_buffer)
            # remove file completely, if too less information
            os.remove(oldpath)
        else:
            missing_frames = self.__rec_frames - self.frames/self.frames_per_buffer
            logging.info("finished file, " +
                         str(missing_frames) + " Frames missing.")
            path = self._get_new_filename(
                broken=True, setNewTime=False, frames=self.frames/self.frames_per_buffer)
            os.rename(oldpath, path)
            logging.debug("Rename Path: " + path)
            # Zip file
            os.system("gzip " + path)
            ...
            if r.CheckForFinished():
                while True:
                    ...
                    if lu > 150:  # lu > 50 and lu < 30:
                        logging.debug(
                            "Bluetooth-Connection possibly lost: %s*t_Callback." % (str(lu)))
